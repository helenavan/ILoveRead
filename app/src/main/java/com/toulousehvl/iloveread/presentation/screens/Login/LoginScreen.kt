...
fun Content(
    paddingValues: PaddingValues,
    signInStateFlow: MutableSharedFlow<APIResult<AuthResult>>,
    resetPasswordStateFlow: MutableSharedFlow<APIResult<Any>>,
    onRegisterNow: () -> Unit,
...
@Composable
fun LoginInState(
    flow: MutableSharedFlow<APIResult<AuthResult>>,
    onSuccess: () -> Unit,
    onError: () -> Unit
) {
    ...
    LaunchedEffect(Unit) {
        flow.collect {
            when (it) {
                ...
                is APIResult.Error -> {
                    Log.e("Login state -> ", it.exception)
...
fun ResetPasswordState(
    flow: MutableSharedFlow<APIResult<Any>>,
    onSuccess: () -> Unit,
    onError: () -> Unit
) {
    val isLoading = remember { mutableStateOf(false) }
    ...
    LaunchedEffect(Unit) {
        flow.collect {
            when (it) {
                ...
                is APIResult.Error -> {
                    Log.e("Reset password state -> ", it.exception)
                    isLoading.value = false
                    onError()
                }
